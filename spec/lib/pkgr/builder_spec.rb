...
describe Pkgr::Builder do
  ...
  describe "#package" do
    ...
    it "builds the proper fpm command" do
      ...
      expect(command).to include("fpm -t deb -s dir --verbose --force -C \"#{builder.build_dir}\" -n \"my-app\" --version \"0.0.1\" --epoch \"#{config.epoch}\" --url \"#{config.homepage}\" --provides \"my-app\"")
    end
